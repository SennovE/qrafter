...
func TestDDLCreateTableRendersColumnReferencesAsTableConstraints(t *testing.T) {
	sql, err := ddl.CreateTable("members").
		Columns(
			ddl.Column("org_id", ddl.BigInt()).References("orgs", "id"),
		).
		Render(dialect.PostgreSQL{})

	require.NoError(t, err)
	assert.Equal(t, `CREATE TABLE "members" (
    "org_id" BIGINT,
    CONSTRAINT "fk_members_org_id_orgs_id" FOREIGN KEY ("org_id") REFERENCES "orgs" ("id")
)`, sql)
}
...
func TestDDLGeneratedColumnsPostgreSQL(t *testing.T) {
	sql, err := ddl.CreateTable("users").
		Columns(
			ddl.Column("id", ddl.BigInt()).IdentityByDefault().NotNull(),
			ddl.Column("full_name", ddl.Text()).GeneratedStored("first_name || ' ' || last_name"),
		).
		Render(dialect.PostgreSQL{})
